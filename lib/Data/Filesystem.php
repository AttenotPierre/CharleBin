...
// Add some meta information not contained in file.
    ...
    {
        return is_file($this->_dataid2discussionpath($pasteid) .
    ...
    public function setValue($value, $namespace, $key = '')
    {
        switch ($namespace) {
            case 'purge_limiter':
                return $this->_storeString($this->_path . DIRECTORY_SEPARATOR . 'purge_limiter.php', '<?php' . PHP_EOL . '$GLOBALS[\'purge_limiter\'] = ' . $value . ';');
            case 'salt':
                return $this->_storeString($this->_path . DIRECTORY_SEPARATOR . 'salt.php', '<?php # |' . $value . '|');
            case 'traffic_limiter':
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      $this->_last_cache[$key] = $value;

                return $this->_storeString($this->_path . DIRECTORY_SEPARATOR . 'traffic_limiter.php', '<?php' . PHP_EOL . '$GLOBALS[\'traffic_limiter\'] = ' . var_export($this->_last_cache, true) . ';');
        }
        return false;
    }
    ...
    {
        return Json::decode(substr(file_get_contents($filename), strlen(self::PROTECTION_LINE . PHP_EOL)));
    }
    ...
    protected function _getExpiredPastes($batchsize)
    {
        ...
        $opened = 0;
        $limit  = $batchsize * 10;
// try at most 10 times $batchsize pastes before giving up
        $time   = time();
    ...
    {
        try {
            return $this->_storeString($filename, self::PROTECTION_LINE . PHP_EOL . Json::encode($data));
        } catch (Exception $e) {
            return false;
        }
    }
    ...
    private function _storeString($filename, $data)
    {
        ...
        @chmod($filename, 0640);
// protect file from access by other users on the host
        return true;
    }
